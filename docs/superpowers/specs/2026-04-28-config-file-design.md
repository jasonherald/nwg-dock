...
- **`apply_config_change` diff logic** (~10 tests): identical configs → `NoChange`; one restart-required field differs → `RestartRequired { restart_fields: ["multi"], applied: [] }`; multiple restart-required fields differ → `RestartRequired { restart_fields: ["multi", "autohide"], applied: [] }`; only hot-reloadable fields differ → `Applicable { applied: [...] }`; mixed (one restart-required + one hot-reloadable) → `RestartRequired { restart_fields: [...], applied: [...] }`; revert to identical via two-step path → `NoChange`; for each field in the apply-path map, a test that changing it produces the expected entry in `applied`.
